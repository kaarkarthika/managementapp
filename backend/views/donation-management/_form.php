...
<style>
    ...
    #donationmanagement-donationamount{
        text-align: right;
    }
</style>
                    ...
                    <div class="panel-body">
                            ...
                            <div class="row">
                                <div class='col-sm-4 form-group mb_0' >
                                    ...
                                    <?= $form->field($model, 'donation_type')->dropDownList($ar,['prompt'=>'SELECT..',]); ?> 
                                </div> 
                                <div class='col-sm-4 form-group mb_0' >
                                    <?php
                                        if($model->donation_date!=""){
                                            $model->donation_date = date('d-m-Y',strtotime($model->donation_date));
                                            if(strpos($model->donation_date, '1970') ||strpos($model->donation_date, '0000')){
                                                $model->donation_date = "-";
                                            }

                                        }
                                     ?>
                                    <?= $form->field($model, 'donation_date')->textInput(['maxlength' => true]) ?>
                                </div>
                                <div class='col-sm-2 form-group mb_0'>
                                    <?= $form->field($model, 'donationAmount')->textInput(['maxlength' => true,'onkeypress'=>'return isNumberKey(event)']) ?>
                                </div>                          
                            </div>
                            ...
                            <div class="row"> 
                                <div class="col-sm-12 form-group mb_0">
                                    <fieldset class="scheduler-border">
                                        <legend class="scheduler-border">Donor Information</legend> 
                                        
                                            <div class="col-sm-6 form-group mb_0">
                                                <?= $form->field($model, 'donor_name')->textInput(['maxlength' => true]) ?>
                                            </div>
                                            <div class="col-sm-6 form-group mb_0">
                                                <?= $form->field($model, 'contact_number')->textInput(['maxlength' => true,'onkeypress'=>'return isNumberKey(event)']) ?>
                                            </div>
                                            <div class="col-sm-6 form-group mb_0">
                                                <?= $form->field($model, 'donor_address')->textarea(['rows' => 2]) ?>
                                            </div>
                                            <div class="col-sm-6 form-group mb_0">
                                                <?= $form->field($model, 'donor_description')->textarea(['rows' => 2]) ?>
                                            </div>
                                    </fieldset>  
                                </div>              
                            </div>
...
<script type="text/javascript">
    $('body').on('change','#donationmanagement-donation_type',function(){
        $('#refNumber').hide();
        $('#cardDetails').hide(); 
        var ty = $('#donationmanagement-donation_type').val();
        // if(ty=='CARD'){
        //     $('#cardDetails').show();
        //     $('#refNumber').hide();
    ...
    $('input#donationmanagement-donation_date').daterangepicker({
